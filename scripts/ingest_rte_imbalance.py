...
LOG_FILE = LOG_DIR / "imbalance_ingestion.log"

RTE_TOKEN_URL = "https://digital.iservices.rte-france.com/token/oauth/"
RTE_IMBALANCE_URL = "https://digital.iservices.rte-france.com/open_api/balancing_capacity/v5/imbalance"
...
def fetch_imbalance(token: str, start_date: datetime, end_date: datetime) -> list[dict]:
    ...
    try:
        ...
        raise RuntimeError(
            "Erreur API RTE imbalance "
            f"{response.status_code} pour start_date={params['start_date']} "
            f"et end_date={params['end_date']}. Reponse: {response.text}"
        ) from exc

    payload = response.json()
    imbalance_blocks = payload.get("imbalance", [])
    if not imbalance_blocks:
        return []

    return imbalance_blocks[0].get("values", [])


def clean_imbalance(raw_values: list[dict], min_points_per_hour: int) -> pd.DataFrame:
    if not raw_values:
...
def insert_imbalance_data(df: pd.DataFrame) -> int:
    if df.empty:
        return 0
    ...
    query = """
        INSERT INTO imbalance_data (timestamp, value, source)
        VALUES %s
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Recupere les donnees d'imbalance RTE et les insere dans PostgreSQL."
    )
    parser.add_argument(
        "--hours",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Nombre minimum de points requis pour garder une heure.",
    )
    return parser
...
def main() -> None:
    ...
    logging.info(
        "Debut ingestion imbalance RTE: %s -> %s",
        format_rte_datetime(start_date),
        format_rte_datetime(end_date),
    )

    token = get_rte_token()
    raw_values = fetch_imbalance(token, start_date, end_date)
    logging.info("Points bruts imbalance recuperes: %s", len(raw_values))

    cleaned = clean_imbalance(raw_values, args.min_points_per_hour)
    logging.info("Lignes horaires imbalance apres nettoyage: %s", len(cleaned))
    ...
    inserted_count = insert_imbalance_data(cleaned)
    logging.info("Lignes inserees ou mises a jour dans imbalance_data: %s", inserted_count)
    logging.info("Fin ingestion imbalance RTE")
